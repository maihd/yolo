...
void DrawText(StringView text, Font font, Vector2 position)
{
    ...
    glUseProgram((GLuint)Graphics::fontShader.Handle);

    I32 projectionLocation = glGetUniformLocation((GLuint)Graphics::fontShader.Handle, "projection");
    I32 modelLocation = glGetUniformLocation((GLuint)Graphics::fontShader.Handle, "model");
    glUniformMatrix4fv(projectionLocation, 1, false, (float*)&Graphics::projection);
    glUniformMatrix4fv(modelLocation, 1, false, (float*)&model);

    glBindVertexArray((GLuint)Graphics::drawTextBuffer.vertexArray);
    glBindBuffer(GL_ARRAY_BUFFER, (GLuint)Graphics::drawTextBuffer.vertexBuffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, (GLuint)Graphics::drawTextBuffer.indexBuffer);
